...
const ExcelTable: React.FC = () => {
    ...
    setData((prev) => {
      ...
      for (let r = minRow; r <= maxRow; r++) {
        for (let c = minCol; c <= maxCol; c++) {
          updated[r] = { ...updated[r], [`col${c}`]: editValue };
        }
      }
      return updated;
    });
  ...
  const columns = useMemo(() => {
    ...
    for (let g = 0; g < numGroups; g++) {
      const lastBlockIndex = numGroups - 1;

      const headersForGroup =
        g === numGroups - 1 ? stickySubHeaders : subHeaders;
      const colKey = (colIndex: number) => `col${colIndex}`;
  ...
  <div className="modal-overlay">
    <div className="modal">
      <h3>Edit Selected Range</h3>
      <input
        type="text"
        value={editValue}
        onChange={(e) => setEditValue(e.target.value)}
      />
      <button onClick={applyEditToRange}>Apply</button>
                ...
                {showFillHandleHere && (
                  <div
                    ...
                    onDoubleClick={() => {
                      ...
                      const { minRow, maxRow } = getRangeBounds(
                        selectedRange.start,
                        selectedRange.end
                      );
                      const pattern: string[] = [];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const repeatedUsers = Array.from({ length: 24 }, (_, index) => {
            ...
            return (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header, headerIndex) => {
                  ...
                  const isStickyRight = numGroups - 1;
                  const isSticky =
                    isBottom && headerIndex >= bottomHeaderCount - 3;
                  const headerDescriptions: Record<string, string> = {
                    "$/MWh": "Block price",
                    MW: "Block Volume",
                    OCP: "Ofer Control Party",
                    AC: "Available Capability",
                    MSG: "Minimum Stable Generation",
                    AOR: "Acceptable Operational Reason",
                  };
